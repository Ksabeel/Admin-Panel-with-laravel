...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pb-2 mb-3 border-bottom">
	<h2 class="h2">All Roles</h2>
	<a href="{{ route('roles.create') }}" class="btn btn-outline-success">Add New Role</a>
</div>
				...
				<table class="table table-striped table-bordered table-hover" id="dataTable">
					...
					<tbody>
						...
						<tr class="text-center">
							...
							<td>{{ $role->name }}</td>
							<td>{{ $role->slug }}</td>
							<td>{{ substr($role->description, 0, 20) }}  {{ strlen($role->description) > 25 ? '....' : '' }}</td>
							...
							<td>
								...
								<form action="{{ route('roles.destroy', $role->id) }}" id="delete-role-{{ $role->id }}" style="display: none;" method="post">
									@csrf
									@method('DELETE')
								</form>
